...
public class ServerRunner {
    ...
    public static void main(String[] args) {
        ...
        try (ServerSocket serverSocket = new ServerSocket(PORT);
            ...
            int numberGames = inputStream.readInt();
            ...
            for (int j = 0; j < numberGames; j++) {
                                ...
                                case 1 -> {
                                    ...
                                    if (offer.equals("capitulate")) {
                                        ...
                                        if (selectPlayer1 > 0 && selectPlayer1 < 4) {
                                            i = gameService.inputOutputServer(inputStream, outputStream, i, selectPlayer1, j);
                                        } else {
                ...
                gameService.resultGame(outputStream, j);
                LocalDateTime timeEnd = LocalDateTime.now();
                long duration = Duration.between(timeStart, timeEnd).getSeconds();
                String durationGame = "Игра № " + (j + 1) + " длилась: " + String.format("%02d:%02d:%02d", duration / 3600,
                        duration / 60 % 60, duration % 60);
                outputStream.writeUTF(durationGame);
                System.out.println(durationGame);
                System.out.println("---------------------------------");
            }

            gameService.resultMatch(outputStream);
